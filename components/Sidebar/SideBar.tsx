...
const SideBar = () => {
  ...
  return (
    <nav className="h-[100vh] basis-0 md:basis-[15%] xl:basis-[10%] md:px-[10px] py-2 sm:py-[10px] lg:py-[20px] xl:py-[30px]  bg-bgGrey flex flex-col items-center justify-between">
      <button onClick={()=>setOpenMenu(!openMenu)} className=" absolute top-1 right-0 w-[2.5rem] h-[2.5rem] rounded-md border p-2 md:hidden">
      ...
      <div className={`bg-white border flex flex-col items-center gap-[45px] right-0 fixed h-[100vh] top-0 px-2 z-50 md:z-10 ${openMenu ? "-right-[180px] ":""} md:right-auto md:bg-transparent md:border-none `}>
        <Link href={"/"}>
          <div className="w-[3rem] h-[3rem] hidden md:block ">
            <Image src={logo} alt="autonomint-dapp" style={{ width: "100%", height: "100%" }} />
          </div>
        </Link>
        <button onClick={()=>setOpenMenu(!openMenu)} className={`absolute top-1 left-1 w-[2.5rem] h-[2.5rem] rounded-md border p-2 md:hidden `}>
